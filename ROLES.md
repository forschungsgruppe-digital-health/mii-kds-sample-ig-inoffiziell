...
- **Lesepfad (Einstieg):** `index.md` (Beschreibung/Scope) → `use-cases.md`
  (Szenarien) → `data-sets.md` (Datenelemente in fachlicher Sprache).
...
- **Beteiligung:** Issue-Template „Kommentar / Ballot-Eintrag" (Kommentar-Typ +
  Begründung), Kommentierungsphase (siehe `COMMENT_RESOLUTION.md`).
...
- **Lesepfad:** `uml.md` → Logical Model `mii-lm-beispiel-modell` →
  `data-sets.md` inkl. **Model-to-Profile-Mapping**.
...
- **Lesepfad:** `input/fsh/` (profile/extension/valueset/codesystem) →
  `conformance.md` → `DESIGN.md` → `qc/custom.rules.yaml` → `MIGRATION.md`,
  `AGENTS.md`.
...
- **Lesepfad:** `conformance.md` (Must Support, fehlende Daten, Such-API) →
  `CapabilityStatement` → Downloads (`package.tgz`/`full-ig.zip`) → Beispiele →
  `PUBLISHING.md`, `NOTIFICATIONS.md`.
...
|------------------|:-------:|:----------:|:--------:|:-------:|
| `index.md` (Beschreibung/Scope) | ● | ● | ○ | ○ |
| `use-cases.md` | ● | ● | ○ | ○ |
| `data-sets.md` (+ Mapping) | ● | ● | ● | ○ |
| `uml.md` / Logical Model | ○ | ● | ● | ○ |
| `input/fsh/` (Profile etc.) |  | ○ | ● | ○ |
| `conformance.md` (Must Support, Such-API) |  | ○ | ● | ● |
| `security-privacy.md` |  |  | ● | ● |
| `CapabilityStatement` |  |  | ● | ● |
| Downloads / `package.tgz` |  |  | ○ | ● |
| `qc/`, `DESIGN.md`, `MIGRATION.md` |  |  | ● | ○ |
| `PUBLISHING.md`, `NOTIFICATIONS.md` |  |  | ○ | ● |
...
- **Fach → Modell:** `data-sets.md` (Datenelemente fachlich) + `use-cases.md`.
...
- **FHIR → DIZ:** `conformance.md`, `CapabilityStatement`, Downloads, Beispiele.
