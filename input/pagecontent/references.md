### Referenzen

*Relevant für: alle Rollen — Lesepfade siehe [ROLES.md](../../ROLES.md).*

*(Pflichtabschnitt aus dem Manteldokument.)*

- HL7® FHIR® R4 (4.0.1): https://hl7.org/fhir/R4/
- FHIR-Basisprofile Deutschland: https://ig.fhir.de/basisprofile-de/
- MII-Kerndatensatz: https://www.medizininformatik-initiative.de/de/der-kerndatensatz-der-medizininformatik-initiative
- HL7 Guidance for FHIR IG Creation: https://build.fhir.org/ig/FHIR/ig-guidance/
- HL7 `FHIR/sample-ig` (Vorlage): https://github.com/FHIR/sample-ig
- HL7 Europe Base/Core IG: https://hl7.eu/fhir/base
- International Patient Summary (IPS): https://hl7.org/fhir/uv/ips/
- US Core IG (Struktur-Exemplar): https://hl7.org/fhir/us/core/
